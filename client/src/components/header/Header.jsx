...
export default function Header() {
    ...
    return (
        <header className='min-h-screen flex-col items-center justify-center'>
            <nav className={`flex items-center p-1 justify-between shadow fixed top-0 z-10 w-full bg-white duration-200 ${navbar ? "visible": "opacity-0"}`} onMouseEnter={() => setNavbar(true)} onMouseMove={() => setNavbar(true)} onMouseLeave={() => setNavbar(false)}>
